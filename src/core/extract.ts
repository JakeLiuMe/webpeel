...
export async function extractWithLLM(
  ...
  if (!llmApiKey) throw new Error('LLM extraction requires llmApiKey');
  if (!prompt && !schema) throw new Error('LLM extraction requires prompt or schema');
...
export function extractStructured(html: string, options: ExtractOptions): Record<string, any> {
  const $ = load(html);
  ...
  if (options.schema) {
    ...
    for (const [field, spec] of Object.entries(properties) as [string, any][]) {
      if (result[field] !== undefined) continue; // Already extracted by selector
