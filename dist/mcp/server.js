...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === 'webpeel_search') {
            const { query, count, provider, searchApiKey: searchKey } = args;
            // SECURITY: Validate input parameters
            ...
            const providerId = provider && validProviders.includes(provider)
                ? provider
                : 'duckduckgo';
        ...
        if (name === 'webpeel_answer') {
            const { question, searchProvider: sp, searchApiKey: sak, llmProvider: lp, llmApiKey: lak, llmModel: lm, maxSources: ms, } = args;
            // SECURITY: Validate input parameters
            ...
            if (!lp || !validLlmProviders.includes(lp)) {
                throw new Error('Invalid llmProvider parameter: must be openai, anthropic, or google');
            }
            if (!lak || typeof lak !== 'string') {
                throw new Error('Invalid llmApiKey parameter: must be a string');
            }
            const validSearchProviders = ['duckduckgo', 'brave'];
            const resolvedSp = sp && validSearchProviders.includes(sp)
                ? sp
                : 'duckduckgo';
